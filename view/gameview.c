...
void PastPlayAnalysis(GameView gv) {
	...
	while (play != NULL) {
		...
		if (character != PLAYER_DRACULA) {
			...
			// Adjust the RESTCHECK, or adjust the previous location. What do you store?
			...
			// ENCOUNTERS ANALYSIS. Include double back
			...
			// Adjust Draculas & HUNTERS_HEALTH after trap encounter
			...
			// This is for Dracula
			// HAVE NOT CONSIDERED THE DOUBLEBACK MOVE!
			// Location function
			// changed NOT_HI_DB_MOVE(Loc)
			round ++;
			if (Loc != HIDE) {
			...
			// ENCOUNTERS ANALYSIS. This won't work for the dracula count of DXY.... where the 
